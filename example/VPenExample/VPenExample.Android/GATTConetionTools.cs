...
namespace VPenExample.Droid
{
    ...
    {
        public GATTConetionTools() : base()
        {
        }

        public GATTConetionTools(IntPtr javaReference, JniHandleOwnership transfer) : base(javaReference, transfer)
        {
        }
        ...
        public byte[] CurrentBlock { get; private set; }

        public async Task<byte[]> ReadBlockWithNotificationWithHeaderAsync(UUID uuid, TimeSpan timeout, OperationToken token, int singleBlockSize, Func<byte[], int> blockSizeSelector)
        {
            if (this.Gatt == null) throw new Exception("GATT is not connected!");
            ReadUUID = uuid;
            ReadStream?.Dispose();
            ...
            {
                int totalBlocks = 2;
                int fullSize = singleBlockSize * totalBlocks;
                ...
                await Task.Run(() =>
                {
                    ...
                    {
                        if (!sizeCalculated && this.CurrentBlock != null)
                        {
                            fullSize = singleBlockSize * blockSizeSelector.Invoke(CurrentBlock);
                            sizeCalculated = true;
                        }
                        ...
                        if (token != null && fullSize > 0)
                        {
                            token.Progress = position / (double)fullSize;
                        }
                ...
                if (ReadStream != null && ReadStream.Position >= fullSize)
